...
import { getLogistics } from 'apiSdk/logistics';
import { LogisticsInterface } from 'interfaces/logistics';

import { LogisticsListPage as ShowCaseList } from 'pages/logistics';
...
function DashboardPage() {
  ...
  const logisticsStat = useSWR<PaginatedInterface<LogisticsInterface>>(
    () => `/logistics`,
    () => getLogistics({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Logistics',
      result: logisticsStat?.data?.totalCount,
      isLoading: logisticsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`logistics`} passHref legacyBehavior>
                  <Button
